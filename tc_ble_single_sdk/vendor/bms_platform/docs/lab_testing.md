...
产物为 `build/lab_firmware/telink_bms.bin`。下载到官方 TLSR8251 开发板后，设备应广播名称 `Telink BMS`，扫描响应包含固定的 BMS Service UUID。模拟器每 500 ms 发布 20S、4 路温度和约 74 V 总压；单体电压、总压和温度在 20 秒周期内连续变化，电流则在 +400 mA 至 +1920 mA、-400 mA 至 -1920 mA 间切换。数值仅用于验证显示、单位、分片、SOC、保护、均衡请求、事件和参数回读；不是 AFE 标定值。
...
使用 `build/pc_client/TelinkBMS.exe` 时，点击“扫描”，双击名称为 `Telink BMS`（或你已保存的自定义名称）的行即可连接。无需手输地址；Windows 现在显示广播中的可配置名称。连接后上位机默认每秒更新仪表盘、20 节单体、4 路温度与故障页；不要把首次连接时的静态参数读数误认为实时数据。

## OTA 实验

先用 Telink 下载器确认官方开发板 Flash 容量为 512 KiB，再执行：
...
2. 等待开发板自动重启并重新广播，然后点击“扫描”，双击 `Telink BMS`（或已保存的自定义名称）重新连接并点击“刷新”。
...
已完成 0.2.1 OTA 验证的开发板可继续执行：

    python tools/bms.py build-lab-ota-control-firmware

在新版上位机 OTA 页选择 `build/lab_ota_control_firmware/telink_bms.bin`，成功重启重连后“固件”为 `0.2.2`。仪表盘采样时间、总压和电流应每秒变化；参数页应显示完整 21 项中文名称、单位、范围和“读/写/持久”。点击“蓝牙名”，输入例如 `BMS 测试` 并确认，断开、扫描后可见新广播名；重新上电后仍必须是 `BMS 测试`。同样，可将 `0x0101` 写为合规的不同值、重新上电后读回，确认参数持久化。

持久化测试只允许使用稳定 USB 供电的官方 512 KiB 开发板。实验固件将记录交替写入 `0x72000` 和 `0x73000` 的两个扇区，并保留上一代有效 CRC 记录；不得把这些地址用于真实 BMS 或未知 Flash 板。
